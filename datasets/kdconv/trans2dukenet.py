...
add_id2query(id2query, query2id, test[:all_test//2], 'ts_seen')
add_id2query(id2query, query2id, test[all_test//2:], 'ts_unseen')
...
for key, _ in id2query.items():
    if int(key.split('_')[-1])%2==0:
        if key[:2] == 'tr':
            split_f.write(key+'\t'+'train'+'\n')
        elif key[:2] == 'va':
            split_f.write(key+'\t'+'dev'+'\n')
        elif key[:7] == 'ts_seen':
            split_f.write(key+'\t'+'test_seen'+'\n')
        elif key[:9] == 'ts_unseen':
            split_f.write(key+'\t'+'test_unseen'+'\n')
split_f.close()
...
for dataset in all_data:
    for dial in dataset:
        ...
        for utte in dial['messages']:
            ...
            history.append(q_id)
print('max candidate len:', max_len)