...
kdconv_dir = os.path.join(datasets_dir, 'kdconv')
kg_path = os.path.join(kdconv_dir, '{}/kb_{}.json'.format(domain, domain))
...
def load_data(domain, type):
    folder_path = os.path.join(kdconv_dir, domain)
...
def pad_sentences(conversations, kgs_index, kgs, kgs_hr_length, kgs_hrt_length, max_sentence_length=50, max_conversation_length=35, max_kg_length=28):
    ...
    for idx, conversation in enumerate(conversations):
        ...
        if len(kgs[idx]) > max_kg_length:
            kgs[idx] = kgs[idx][:max_kg_length]
            kgs_hr_length[idx] = kgs_hr_length[idx][:max_kg_length]
            kgs_hrt_length[idx] = kgs_hrt_length[idx][:max_kg_length]
        ...
        kgs[idx] = pad_conversation(kgs[idx])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--max_conversation_length', type=int, default=50)
